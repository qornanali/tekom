...
void expression(void){
	if (isplus || isminus) get_token(&token, infile);

	factor();
	while (isplus || isminus || isdiv || isTimes) {
		get_token(&token, infile);
		factor();
	}
}
...
void factor(void){
	if (isNumbr || isIdentifier) {
		if(isIdentifier){
			alamat a;
			if( (a = search(list1, token.charvalue)) == NULL){
...
void outblock(void){
	...
	while(isProcedure || isFunction){
		...
		if(!isIdentifier){
			...
			push(&list1, &token.charvalue[0], FNAME, NULL);
...
int main(int argc, char * argv[]){
	...
	//nama file input
	char namafile[65] = "Praktikum 8.txt";
