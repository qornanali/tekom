...
void statement (void){
	if(isIdentifier){
		...
		gencode(JPC,0,0,line+3 + NESTED_IFWHILE);
		
		gabung();
		
		gencode(JMP,0,0, x + NESTED_IFWHILE);
		NESTED_IFWHILE--;
	}else if(isIf){
		...
		if(isElse){
			...
			gencode(JMP,0,0,line+2+NESTED_IFWHILE);
		}else{
			gencode(JPC,0,0,line+2+NESTED_IFWHILE);
		}
...
int main(int argc, char * argv[]){
	...
	//nama file input
	char namafile[65] = "Praktikum 12.txt";
