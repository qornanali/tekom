	...
	if (isNumbr || isIdentifier) {
		if(isIdentifier){
			...
			if(a->type == FNAME){
				gencode(INT,0,0,1);
				function(a);
				gencode(CAL,0,0,a->address);
				gencode(INT,0,0,-a->nparam);
			}else{
...
void inblock(void){
	...
	if(isVar){
		...
		if(!isIdentifier){
			...
			//IDENTIFIER yang diambil harus dimasukkan kedalam stack sebagai variable lokal
			push(&list1, &token.charvalue[0], VLOCAL, NULL, jumlahVariable);
			jumlahVariable++;
		}
...
void outblock(void){
	...
	if(isProcedure || isFunction){
		...
		while(isProcedure || isFunction){
			...
			char temp[30], inblockType = PNAME;
			...
			if(!isIdentifier){
				...
				push(&list1, &token.charvalue[0], inblockType, NULL, line+2);
...
int main(int argc, char * argv[]){
	...
	//nama file input
	char namafile[65] = "Praktikum 14.txt";
