          ...
          {     
              if(!issemicolon)
              {
                char error[50];
                 switch(token.attr)
                 {
                  case 1: strcpy(error,"identifier");break;
                  case 2: strcpy(error,"number");break;
                  case 3: strcpy(error,"reserved Word");break;
                  case 4: strcpy(error,"symbol");break; 
                 }
                 printf("Fatal: Syntax Error, ';' expected but '%s %s' found\n",error,token.charvalue);
                 printf("Fatal: Compilation Aborted\n");
                 exit(-1);
